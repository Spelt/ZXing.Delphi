...
begin
  ...
  if (((dimension >= 8) and (dimension <= 144)) and ((dimension and $01) = 0)) then
  begin
    self.Fversion := readVersion(bitMatrix);
    if (self.version <> nil) then
    begin
      self.mappingBitMatrix  := extractDataRegion(bitMatrix);
      self.readMappingMatrix := TBitMatrix.Create(self.mappingBitMatrix.Width, self.mappingBitMatrix.Height)
    end;
  end;
end;
...
begin
  ...
  begin
	...
	begin
	  if ((row = (numRows - 2)) and (column = 0) and ((numColumns and $03) <> 0) and (not corner2Read)) then
      begin
      ...
      begin
        if ((row = (numRows + 4)) and (column = 2) and ((numColumns and $07) = 0) and (not corner3Read)) then
        begin
        ...
        begin
          if ((row = (numRows - 2)) and (column = 0) and ((numColumns and $07) = 4) and (not corner4Read)) then
          begin
          ...
          begin
            ...
            begin
              if ((row < numRows) and (column >= 0) and (not readMappingMatrix[column, row])) then
              begin
            ...
            begin
              if ((row >= 0) and (column < numColumns) and (not readMappingMatrix[column, row])) then
              begin
                result[resultOffset] := Byte(readUtah(row, column, numRows, numColumns));
                Inc(resultOffset);
              end;
...
function TBitMatrixParser.readModule(row, column, 
  numRows, numColumns: Integer): Boolean;
begin
...
function TBitMatrixParser.readUtah(row, column, 
  numRows, numColumns: Integer): Integer;
var
  currentByte : Byte;
begin
  currentByte := 0;
...
var
  ...
  numDataRegionsRow,
  numDataRegionsColumn,
  sizeDataRegionRow,
  sizeDataRegionColumn,
  dataRegionRow,
  dataRegionRowOffset,
  dataRegionColumn,
  dataRegionColumnOffset,
  readRowOffset,
  writeRowOffset,
  readColumnOffset,
...
begin
  ...
  then
     raise EArgumentException.Create('Dimension of bitMarix must match the version size');
  ...
  bitMatrixWithoutAlignment := TBitMatrix.Create(sizeDataRegionColumn, sizeDataRegionRow);
