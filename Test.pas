...
procedure TZXingDelphiTest.AllEAN_13();
var
  result: TReadResult;
begin
  try
    result := Decode('EAN13.png', TBarcodeFormat.EAN_13);
    Assert.IsNotNull(result, ' nil result ');
    Assert.IsTrue(result.Text.Equals('16466265651114'),
      'EAN_13 result Text Incorrect: ' + result.Text);

    //result := Decode('upca.png', TBarcodeFormat.UPC_A);
    //Assert.IsNotNull(result, ' nil result ');
    //Assert.IsTrue(result.Text.Equals('123456789012'),
    //  'UPC_A result Text Incorrect: ' + result.Text);
  finally
    FreeAndNil(result);
  end;
end;
...
begin
  try
    ...
    result := Decode('EAN13.png', TBarcodeFormat.Auto);
    Assert.IsNotNull(result, ' nil result ');
    Assert.IsTrue(result.Text.Equals('16466265651114'),
      'EAN_13 result Text Incorrect: ' + result.Text);
