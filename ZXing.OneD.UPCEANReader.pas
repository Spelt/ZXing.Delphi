...
type
  ...
  private
    ...
    class function checkStandardUPCEANChecksum(const s: String): Boolean; static;
...
var
  i, j : Integer;
  widths,
  reversedWidths : TArray<Integer>;
begin
  MAX_AVG_VARIANCE := Trunc(TOneDReader.PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.48);
  MAX_INDIVIDUAL_VARIANCE := Trunc(TOneDReader.PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.7);

  START_END_PATTERN := TArray<Integer>.Create(1, 1, 1, 1);
  MIDDLE_PATTERN := TArray<Integer>.Create(1, 1, 1, 1, 1);
  END_PATTERN := TArray<Integer>.Create(1, 1, 1, 1, 1, 1);

  L_PATTERNS := TArray<TArray<Integer>>.Create(
                  TArray<Integer>.Create(3, 2, 1, 1), // 0
...
begin
  ...
  SetLength(counters, Length(START_END_PATTERN));
  while (not foundStart) do
  begin
    for idx := 0 to Pred(Length(START_END_PATTERN)) do
      counters[idx] := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (patternMatchVariance(counters, pattern, MAX_INDIVIDUAL_VARIANCE) < MAX_AVG_VARIANCE) then
        begin
...
begin
  ...
  digit := -1;
  if (not TOneDReader.recordPattern(row, rowOffset, counters))
  then
     exit;
...
begin
  ...
  if (startGuardRange=nil) // Sometimes startGuardRange are NIL! without a Exit, you will run into a exception!
  then
    exit;

  if Assigned(resultPointCallback) then
  begin
    resPoint := TResultPointHelpers.CreateResultPoint((startGuardRange[0]+startGuardRange[1]) div 2, rowNumber);
  ...
  if ((quietEnd >= row.Size) or not row.isRange(ending, quietEnd, false))
  then
     exit;
  resultString := res.ToString;
  if (Length(resultString) < 8)
  then
     exit;
  if (not checkChecksum(resultString))
  then
     exit;
  ...
  begin
    ...
    if (hints <> nil) and (hints.ContainsKey(TDecodeHintType.ALLOWED_EAN_EXTENSIONS))
    then
       allowedExtensions := TArray<Integer>(hints[TDecodeHintType.ALLOWED_EAN_EXTENSIONS])
    else
